...
def B1_Frontend():
    ...
    titulo.markdown('<h1 style="text-align: center; margin-top: 300px; color: white;">¿Quieres saber qué nos dicen los datos de la industria de los satélites?</h1>', unsafe_allow_html=True)
    
    window = st.radio("",["***DATOS***", "***CONSECUENCIAS***"],index=None, horizontal = True)

    if window == '***DATOS***':
        st.subheader("Desde los años 50 el ser humano ha incrementado exponencialmente el número de satélites lanzados. Con el paso de los años cada vez es mayor nuestra evolución tecnológica en comunicación, meteorología, recogidas de datos, necesidad de investigación del espacio exterior...")
        fig1 = B1_01()
        st.plotly_chart(fig1)
        st.write("\n\n")
        st.subheader("Este avance tecnológico también queda reflejado en el crecimiento del tiempo de vida de los satélites lanzandos:")
        appointment = st.slider("Marque la fecha en la que esté interesado: ",min_value = 1978,max_value=1998, value=1980)
...
def B1_01():
    ...
    #Visualización
    fig = px.line(df_por_año,x="Date of Launch",y="count",labels={"Date of Launch":"Año de Lanzamiento","count":"Número de lanzamientos"},
...
def B1_03():
    ...
    fig = px.bar(df_frecuencias, x= 'Date of Launch', y='Frecuencia Acum', labels= {"Date of Launch":"Año","Frecuencia Acum":"Acumulación"},title='Acumulación de satélites')
