...
def B4_01():
    ...
    fig = px.bar(count_orbit_df, labels={'value': 'Número de satélites', 'index': 'Clase de órbita'}, title='Número de satélites por clase de órbita')
    return fig

def B4_02():
    global DF
    count_orbit_df = DF.groupby('Type of Orbit')['Type of Orbit'].count()
    fig = px.bar(count_orbit_df, labels={'value': 'Número de satélites', 'index': 'Tipo de órbita'}, title='Número de satélites por tipo de órbita')
    return fig
...
def B4_Frontend():
    ...
    st.write("Decimos que una órbita pertenece a una clase u otra en función de su <b>distancia respecto a la Tierra </b>. En esta clasificación sólo es reletante la distancia y la forma.")
    st.write("<h3>LEO</h3>",unsafe_allow_html=True)
    st.write("LEO (<i>Low Earth Orbit</i>) es la órbita con menor distancia a la tierra, en un rango de 160 a 700kms de radio. "
             ...
             " Este tipo de satélites alcanzan hasta 7.8 kms/s, lo que les permite dar una vuelta a la Tierra en ~90 minutos.",unsafe_allow_html=True)
    ...
    st.write("MEO (<i>Medium Earth Orbit</i>) es una órbita generalmente con un radio más amplio que la LEO y con <i>mayor diversidad en la orientación</i>. MEO abarca órbitas de 500 a 1000kms de radio. "
             ...
             " Como se aprecia en la animación, su periodo es ligeramente más lento que el de LEO por la diferencia en radio.",unsafe_allow_html=True)
    ani = plot(MEO, title='Órbita MEO', animate=True)
    components.html(ani.to_jshtml(), height=600)
    plt.clf()
    st.write("<h3>GEO</h3>",unsafe_allow_html=True)
    st.write("GEO (<i>Geostationary Orbit</i>) es una órbita con el radio más amplio entre LEO y MEO, con un periodo de 23 horas 56 minutos y 4 segundos. "
             ...
             " como European Galileo System, utilizado en la comunicación europea."
             " Como se aprecia en la animación, su periodo es mucho más lento que el de LEO y MEO por la diferencia en radio.",unsafe_allow_html=True)
    ani = plot(GEO, title='Órbita GEO', animate=True)
    components.html(ani.to_jshtml(), height=600)
    plt.clf()
    st.write("<h3>Elliptical</h3>",unsafe_allow_html=True)
    st.write("Las órbitas elípticas se utilizan para satélites muy especializados y fuera de los ámbitos más comunes como telecomunicaciones y navegación. "
